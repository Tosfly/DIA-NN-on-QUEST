...
Place both shell scripts, both FASTA files, and all 16 RAW files in the same project directory. From that directory, submit the library-generation job:

```bash
sbatch Lib_Gen_Mouse_Phos.sh
```

After the library job completes successfully and the predicted `.speclib` file is present, submit the DIA search:

```bash
sbatch DIA_MS_Phos_search.sh
```

Check either job with:
...
bash -n Lib_Gen_Mouse_Phos.sh
bash -n DIA_MS_Phos_search.sh
...
```bash
OVERWRITE=1 sbatch Lib_Gen_Mouse_Phos.sh
```

or:

```bash
OVERWRITE=1 sbatch DIA_MS_Phos_search.sh
```
...
| `--min-pr-charge` / `--max-pr-charge` | 2 / 4 | Phosphopeptide precursor-charge range |
